...
export default {
  ...
  methods: {
    ontabtap(e) {
      ...
      this.isTap = true
      const currentSize = this.tabListSize[index]
    ...
    onswiperscroll(e) {
      ...
      let preloadIndex = this._lastTabIndex
      ...
      // #ifdef APP-PLUS || H5 || MP-WEIXIN || MP-QQ
      const percentage = Math.abs(this.swiperWidth / offsetX)
      const currentSize = this.tabListSize[this._lastTabIndex]
    ...
    selectorQuery() {
      ...
      const queryTabSize = uni.createSelectorQuery().in(this)
      for (let i = 0; i < this.tabList.length; i++) {
    ...
    switchTab(index) {
      ...
      if (this.pageList[this.tabIndex].dataList.length > MAX_CACHE_DATA) {
        const isExist = this.cacheTab.indexOf(this.tabIndex)
        if (isExist < 0) {
      ...
      if (this.cacheTab.length > MAX_CACHE_PAGE) {
        const cacheIndex = this.cacheTab[0]
